...
async function registerAllPanelsAndCommands(context: vscode.ExtensionContext): Promise<void> {
    ...
    // Check for pipeline session in workspace (after a short delay to let kernel start)
    setTimeout(() => {
        detectAndPromptPipelineSession(context, state).catch((error) => {
            logger.error('Failed to detect pipeline session:', error);
        });
    }, 2000);
}

/**
 * Detect pipeline session file in workspace and prompt user to load it
 *
 * This runs after extension activation to check if the workspace contains
 * a squiggy-session.json file (typically generated by the aa-tRNA pipeline).
 */
async function detectAndPromptPipelineSession(
    context: vscode.ExtensionContext,
    state: ExtensionState
): Promise<void> {
    // Check user preference first
    const preference = getAutoLoadPreference(context);
    if (preference === 'skip') {
        logger.debug('Pipeline session auto-detection disabled by user preference');
        return;
    }

    // Try to detect pipeline session
    const detection = await detectPipelineSession();
    if (!detection.found || !detection.session || !detection.sessionDir) {
        logger.debug('No pipeline session found in workspace');
        return;
    }
    ...
    if (response === 'Load Session') {
        ...
        await vscode.window.withProgress(
            ...
            async (progress) => {
                ...
                await state.fromSessionState(resolvedSession, context);
            }
        );

        statusBarMessenger.show(`Loaded: ${sessionName}`, 'folder-library');
        logger.info(`Pipeline session loaded: ${sessionName}`);
