    ...
    for idx, (read_id, signal, sample_rate) in enumerate(reads_data):
        ...
        if show_dwell_time:
            ...
            for i, base_annotation in enumerate(base_annotations):
                ...
                # Use pre-calculated dwell time from annotation if available
                if base_annotation.dwell_time_ms is not None:
                    dwell_time = base_annotation.dwell_time_ms
                ...
                for sample_offset in range(0, dwell_samples, downsample):
                    ...
                    if sample_idx < len(signal):
                        ...
                        signal_dwell_times.append(dwell_time)
            ...
            for i, base_annotation in enumerate(base_annotations):
                ...
                # Calculate dwell time for tooltip
                dwell_time = (num_samples / sample_rate) * 1000  # ms
                # Use pre-calculated dwell time from annotation if available
                if base_annotation.dwell_time_ms is not None:
                    dwell_time = base_annotation.dwell_time_ms
